...
| Main routes (curated line roster, `data/main_routes.json`) | derived | The 21 marquee corridors (16 street lines + 5 trail lines) drawn as major routes on both maps — each a named corridor end-to-end with facility-grade mileage (off-street / protected / painted / none) computed along its length. The roster is editorial: we chose which corridors count as main routes; segment grades and mileage are computed from CDOT Bike Routes + OSM trails each run. Corridor gaps stay holes — geometry is never fabricated; street lines (derived, from CDOT) and trail lines (crowdsourced, from OSM) never blend tiers. On the network map, each line renders in one solid color end to end (facility grade is an opt-in border layer instead); the transportation map keeps per-segment grade coloring. |
