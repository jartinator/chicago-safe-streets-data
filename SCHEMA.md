# SCHEMA.md — published data contracts (v1.1)
...
| `crowdsourced` | Community-curated, unverified (reserved: Mellow Bike Map) |
...
## planned_routes.geojson / mellow_routes.geojson — stubs
`{ type: "FeatureCollection", features: [], properties: { status: "no_data_yet", note } }`
Populate with the same LineString schema as `bike_routes.geojson` (planned:
`facility_category` may be `"other"`; mellow: set `data_tier: "crowdsourced"`).
