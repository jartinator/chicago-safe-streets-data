...
in the pipeline. **TODO:** `site/data/meta.json` on disk still carries
`"contract_version": "1.9"` — it was last regenerated by the network-tiers-v2
pipeline commit before `CONTRACT_VERSION` was bumped here, so it now
undercounts the contract version by one. It otherwise already reflects this
round's outputs (19-line roster, `mellow_connectors` source entry) — a full
pipeline run (`python3 pipeline/run_all.py` or `refresh_reporting.py`) is
needed to pick up the corrected `contract_version` string. Not regenerated
...
count. When zero parts survive, `features` is `[]` (no zero-part feature is
ever emitted). When `mellow_routes.geojson` itself had no features to dedupe
this run, the file still has the full envelope shape with `features: []` and
