...
def test_roster_shape_matches_spec():
    ...
    # vincennes demoted to connectors. DECISIONS.md #26 adds 312-riverrun as
    # a 6th trail line -> 20.
    lines = ROSTER["lines"]
    assert len(lines) == 20
    street = [ln for ln in lines if ln["source"] == "bike_routes"]
    trail = [ln for ln in lines if ln["source"] == "osm_trails"]
    assert len(street) == 14 and len(trail) == 6
    ...
    assert {"lakefront", "bloomingdale", "major-taylor",
            "north-shore-channel", "north-branch", "312-riverrun"} <= set(ids)
