...
# paragraph verbatim — that paragraph describes the whole human site,
# including the obstruction map layer, which is synthetic and explicitly
# EXCLUDED from this API (see NO_SYNTHETIC_DATA_STATEMENT); describing this
# API's actual contents avoids implying obstruction data is available here.
PROJECT_DESCRIPTION = (
...
# sitemap order. obstructions-preview.html is deliberately excluded — it's a
# gated synthetic-demo page, never a citable part of the site. Keep this in
# sync with the actual site/*.html file list (see build_sitemap_xml).
_SITEMAP_PAGES = [
