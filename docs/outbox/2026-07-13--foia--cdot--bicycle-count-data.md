...
To: FOIA Officer, Chicago Department of Transportation

Re: Freedom of Information Act request — bicycle count data

Dear FOIA Officer,

Under the Illinois Freedom of Information Act, 5 ILCS 140, I request copies
of the following. Each item is independent — a partial response is welcome,
and for any dataset an export of the existing spreadsheet or database in its
native format is preferable to any newly created document.

1. The bicycle count data underlying CDOT's February 25, 2011 news release
   "CDOT Bicycle Count Study" — the location-level counts from the automated
   pneumatic tube counters deployed at 26 locations citywide in summer/fall
   2009, in the form maintained.

2. Any subsequent CDOT bicycle count data from 2010 to the present,
   including manual or volunteer counts (for example, the spring 2015
   weekday counts at Chicago Avenue & Wells Street referenced in public
   reporting) and any automated counts, in the form maintained.
...
Separately from this request: if bicycle count data is something CDOT would
consider publishing on the City's data portal on an ongoing basis (as New
York City and Washington, DC do for their bicycle counters), we would
enthusiastically use and publicize it, and we are happy to share a short
note on dataset structures that have worked well in other cities — offered
as a resource, not a prescription. A recurring portal dataset would also
spare your office future one-off requests like this one.
