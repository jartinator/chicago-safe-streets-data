(function () {
  ...
  const SOURCES = [
    ...
    },
    {
      ...
      tier: "derived",
      cadence: "weekly pipeline run",
      description: "19 marquee corridors (14 street lines + 5 trail lines, owner-signed roster) drawn heavy on the map. A \"line\" is a named corridor end-to-end (Halsted: 79th ⇄ Waveland) drawn in one solid color; an opt-in quality border shows the facility grade along it (protected / paint / mellow / none — trails are off-street). The line list is hand-picked in a checked-in roster (data/main_routes.json); each pipeline run auto-fills every line with the real CDOT/OSM segments that match it, so grades and mileage stay live.",
