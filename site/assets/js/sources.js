(function () {
  ...
  const SOURCES = [
    ...
    },
    {
      ...
      tier: "derived",
      cadence: "weekly pipeline run",
      description: "The marquee corridors (owner-signed roster of street lines and trail lines) drawn heavy on both map screens. A \"line\" is a named corridor end-to-end (Halsted: 79th ⇄ Waveland). The network map is a deliberate schematic: it straightens each line into one continuous spine and draws quality as structural ink along it — off-street / protected / paint / nothing (a hollow stretch marks where no bikeway is built) — while the transportation map colors each member segment directly by its own grade in true geometry, so quality reads segment-by-segment. The line list is hand-picked in a checked-in roster (data/main_routes.json); each pipeline run auto-fills every line with the real CDOT/OSM segments that match it, so grades and mileage stay live.",
